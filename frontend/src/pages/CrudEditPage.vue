...
function applyJsonFromEditor() {
  if (monacoEditor) {
    try {
      ...
      // Garantir que fields tenham layout properties e readonly default
      ...
      // Garantir que campos em layouts também tenham readonly default
      ...
      // Garantir que campos em relations também tenham readonly default
      if (config.value.relations) {
        config.value.relations.forEach(relation => {
          if (relation.fields) {
...
function addFieldToUnassigned() {
  const field = {
    ...
    default: '',
    skipIfEmpty: false,
    readonly: newField.value.readonly !== undefined ? newField.value.readonly : false,
    layoutIndex: -1,
    row: -1,
    col: -1,
    index: -1
  ...
  newField.value = { name: '', label: '', type: 'text', readonly: false };
}
...
async function loadCrud() {
  try {
    ...
    if (response.data.config) {
      ...
      // Normalizar readonly para false se não existir
      if (config.value.fields) {
        config.value.fields.forEach(field => {
