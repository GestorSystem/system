...
const filterableRelations = computed(() => {
  if (!props.config.relations || !Array.isArray(props.config.relations)) {
    return [];
  }
  const relations = props.config.relations.filter(relation => 
...
async function fetchItems() {
  ...
  try {
    ...
    // Adicionar filtros de relations
    Object.keys(relationFilters.value).forEach(key => {
      const value = relationFilters.value[key];
...
function onFiltersChange(filters) {
  ...
  pagination.value.page = 1;
  // Buscar novamente com os novos filtros
  fetchItems();
}
