          ...
          <q-form @submit="saveItem" class="q-gutter-md">
            ...
            <template v-for="field in formFields" :key="field.name">
              <RenderField
                ...
                :select-options="dialogSelectOptions"
                :options-map="dialogOptionsMap"
                :loading-map="dialogLoadingMap"
                @file-upload="handleFileUpload"
                @update:model-value="(payload) => currentItem[payload.field] = payload.value"
              />
            </template>
            
            <!-- Renderizar relations do CRUD referenciado -->
            <template v-if="dialogRelations && dialogRelations.length > 0">
              <div v-for="(relation, index) in dialogRelations" :key="index" class="q-mb-md">
                  ...
                  <q-card-section class="q-pa-none">
                    ...
                    <q-card-section>
                      <RenderField
                        ...
                        :loading-map="dialogLoadingMap"
                        @file-upload="handleFileUpload"
                        @update:model-value="(payload) => handleDialogRelationUpdate(payload, relation)"
                      />
                    </q-card-section>
                  </q-card-section>
                </q-card>
...
// State para relations e select options (para o dialog)
const dialogSelectOptions = ref({});
const dialogOptionsMap = ref({});
const dialogLoadingMap = ref({});
const dialogRelations = computed(() => {
  if (referencedCrud.value && referencedCrud.value.config && referencedCrud.value.config.relations) {
    const relations = Array.isArray(referencedCrud.value.config.relations) 
      ? referencedCrud.value.config.relations 
      : [referencedCrud.value.config.relations];
    // Filtrar apenas relations que não são inline/sub-crud (essas são tratadas pelo SubCrudTable pai)
    return relations.filter(r => r.type !== 'inline' && r.type !== 'sub-crud');
  }
  return [];
});

async function openDialog(item = null, index = null) {
  ...
  // Carregar relations e select options antes de abrir o dialog
  await loadDialogRelations();
  await loadDialogSelectOptions();
...
// Carregar select options para campos do tipo select
async function loadDialogSelectOptions() {
  if (!formFields.value || !Array.isArray(formFields.value)) {
    return;
  }
  
  const selectFields = formFields.value.filter(field => field.type === 'select' && field.optionsEndpoint);
  
  for (const field of selectFields) {
    try {
      dialogLoadingMap.value[field.name] = true;
      const response = await api.get(field.optionsEndpoint);
      let data = response.data;
      
      // O backend pode retornar { data: [...], count: ... } ou um array direto
      ...
      dialogSelectOptions.value[field.name] = field.transformOptions 
        ? field.transformOptions(data) 
        : data;
    } catch (error) {
      console.error(`Erro ao carregar opções para ${field.name}:`, error);
      dialogSelectOptions.value[field.name] = [];
    } finally {
      dialogLoadingMap.value[field.name] = false;
    }
  }
}

// Carregar relations do CRUD referenciado
async function loadDialogRelations() {
  if (!referencedCrud.value || !referencedCrud.value.config || !referencedCrud.value.config.relations) {
    return;
  }
  ...
  for (const relation of relations) {
    ...
    // Se a relação depende de outro campo, não carregar agora
    if (relation.dependsOn) {
      // Limpar opções anteriores
      dialogOptionsMap.value[relation.field] = [];
      continue;
    }
    
    await loadDialogRelation(relation);
  }
}

// Carregar uma relação específica
async function loadDialogRelation(relation, filterValue = null) {
  // Para relações inline/sub-crud (hasMany), não precisa carregar lista de disponíveis
  if (relation.type === 'inline' || relation.type === 'sub-crud') {
    return;
  }
  
  dialogLoadingMap.value[relation.field] = true;
  try {
    // Construir parâmetros da requisição
    const params = {};
    if (filterValue && relation.filterParam) {
      params[relation.filterParam] = filterValue;
    }
    
    // Fazer requisição para o endpoint da relação
    const endpoint = relation.endpoint || `/api/${relation.modelName || relation.field}`;
    ...
    // Armazenar no optionsMap usando o field da relação como chave
    dialogOptionsMap.value[relation.field] = data;
  } catch (error) {
    console.error(`Erro ao carregar relação ${relation.field}:`, error);
    dialogOptionsMap.value[relation.field] = [];
  } finally {
    dialogLoadingMap.value[relation.field] = false;
  }
}

// Função para lidar com atualizações de relations no dialog
function handleDialogRelationUpdate(payload, relation) {
  // Atualizar o campo da relação (ex: 'State')
  currentItem.value[payload.field] = payload.value;
  ...
  if (relation.payloadField && relation.payloadField !== payload.field) {
    currentItem.value[relation.payloadField] = payload.value;
  }
}
