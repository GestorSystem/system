...
async function saveItem() {
  try {
    ...
    // Remover campos readonly do payload (não devem ser editados)
    ...
    // Limpar valores inválidos (especialmente datas)
    Object.keys(payload).forEach(key => {
      const value = payload[key];
      // Se for string "Invalid date" ou similar, converter para null
      if (typeof value === 'string' && (value.toLowerCase().includes('invalid') || value === 'Invalid date')) {
        payload[key] = null;
      }
      // Se for string vazia, null ou undefined, remover se skipIfEmpty
