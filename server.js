...
class GestorServer {
  ...
  _setupOpenApiMiddleware(apiSpec) {
    const middleware = openApiMiddleware({
      ...
      // Ignorar paths específicos do Swagger UI e alguns caminhos da API
      // para evitar conflitos com o middleware de validação
      ignorePaths: (path) => {
        // Ignorar Swagger UI (/api-docs e sub-rotas)
        if (path.startsWith('/api-docs')) {
          return true;
        }

        // Ignorar alguns caminhos da API (mantendo o comportamento anterior)
        const apiIgnoreRegex = /^\/api\/(models|pessoas|.*)/;
        return apiIgnoreRegex.test(path);
      },
    });
