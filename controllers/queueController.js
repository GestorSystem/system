...
async function updateQueue(req, res) {
  try {
    ...
    // Preparar dados para atualização, validando e limpando valores inválidos
    const updateData = { ...req.body };
    
    // Remover campos readonly que não devem ser atualizados manualmente
    const readonlyFields = ['lastProcessed', 'totalItems', 'totalProcessed', 'totalFailed', 'processing'];
    ...
    // Validar e limpar valores de data inválidos
    if (updateData.lastProcessed !== undefined) {
      if (typeof updateData.lastProcessed === 'string' && 
          (updateData.lastProcessed.toLowerCase().includes('invalid') || 
           updateData.lastProcessed === 'Invalid date')) {
        delete updateData.lastProcessed; // Não atualizar se for inválido
      } else if (updateData.lastProcessed === '') {
        updateData.lastProcessed = null;
      }
    }
    
    // Validar tipos numéricos
    if (updateData.itemsPerBatch !== undefined) {
      const num = parseInt(updateData.itemsPerBatch);
      if (isNaN(num)) {
        delete updateData.itemsPerBatch;
      } else {
        updateData.itemsPerBatch = num;
      }
    }
    
    if (updateData.maxAttempts !== undefined) {
      const num = parseInt(updateData.maxAttempts);
      if (isNaN(num)) {
        delete updateData.maxAttempts;
      } else {
        updateData.maxAttempts = num;
      }
    }
    
    if (updateData.retryDelay !== undefined) {
      const num = parseInt(updateData.retryDelay);
      if (isNaN(num)) {
        delete updateData.retryDelay;
      } else {
        updateData.retryDelay = num;
      }
    }
